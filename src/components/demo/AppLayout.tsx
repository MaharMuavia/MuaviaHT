...
import { Home, Upload, Activity, Lightbulb, Gavel, Play, BarChart3, Orbit } from 'lucide-react';
...
export function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex flex-col min-h-screen max-w-md mx-auto relative bg-navy-bg shadow-2xl">
      ...
      {/* Bottom Navigation */}
      <nav className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-md bg-background/90 backdrop-blur-xl border-t border-white/5 px-2 py-3 flex justify-around items-center z-50">
        {navItems.map((item) => (
          <button
            key={item.id}
